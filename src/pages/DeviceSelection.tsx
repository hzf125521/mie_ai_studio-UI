...
export const DeviceSelection: React.FC = () => {
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      <h1 className="text-3xl font-bold text-gray-900 mb-8">Device Selection & Model Management</h1>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-800">Algorithm List</h2>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Device Code</th>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Operations</th>
              </tr>
            </thead>
            <tbody className="bg-white divide-y divide-gray-200">
              {tableData.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-4 text-center text-gray-500">No models found.</td>
                </tr>
              ) : (
                tableData.map((model) => (
                  <tr key={model.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {model.workflow === 'outliers' ? 'Outliers Detection' : 'Regression Residual'} - {model.type}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${model.isDeployed ? 'bg-green-100 text-green-800' :
                        ...
                        {model.isDeployed ? 'Deployed' : model.statusLabel}
                      </span>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium flex gap-3">
                      ...
                      {(getModelStatus(model) === 'Validated' || isModelDeployed(model)) ? (
                        <button
                          ...
                          title={isModelDeployed(model) ? "Stop Deployment" : "Deploy Model"}
                        >
        ...
        <div className="mt-2">
          ...
          {modelToDelete && (
            <p className="text-sm text-gray-500 mt-2">
              Current status: <span className="font-semibold">{getModelStatus(modelToDelete)}</span>
            </p>
          )}
        </div>
